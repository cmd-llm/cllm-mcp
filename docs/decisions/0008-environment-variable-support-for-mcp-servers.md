...
5. CLLM_MCP_SERVER_ENV_* Variables (environment overrides)
...
5. CLLM_MCP_SERVER_ENV_* Variables
6. CLI Flags
...
3. **CLLM_MCP_SERVER_ENV_* Override System:** The implementation includes a feature for CLI environment variable overrides via special environment variables (CLLM_MCP_SERVER_ENV_{SERVER}_{VAR}), which is not documented in the original ADR. This is a useful feature but adds undocumented behavior.
...
1. **High Priority - User Documentation**
   ...
   - Document the CLLM_MCP_SERVER_ENV_* override system
